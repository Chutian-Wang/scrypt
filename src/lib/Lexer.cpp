    ...
    if (token == '\n') {
      ...
      if (std::isdigit(input.peek()) || (input.peek() == '.')) {
        ...
        if (n == 0) {
          ...
          add_token(TokenType::NUMBER, num, currRow,
                    currCol - (num.length() - 1));
          num = "";
        } else if (n == 1) {
          ...
          if (idx == 0) {
            str += token;
            throw SyntaxError(Token(TokenType::ERR, str, currRow,
                                    currCol - (num.length() - 1)));
            str = "";
          } else if (idx == num.length() - 1) {
            str += token;
            throw SyntaxError(Token(TokenType::ERR, str, currRow, currCol + 1));
            str = "";
          } else {
            add_token(TokenType::NUMBER, num, currRow,
                      currCol - (num.length() - 1));
          ...
          if (idx == 0) {
            ...
            str += token;
            throw SyntaxError(Token(TokenType::ERR, str, currRow, period2));
            str = "";
          }
        }
        temp = 0;
